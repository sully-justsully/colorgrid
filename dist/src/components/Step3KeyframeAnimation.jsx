...
function cubicBezierEase(t) {
    const c1x = 0.22, c1y = 1, c2x = 0.36, c2y = 1;
    function cubicBezier(t, p0, p1, p2, p3) {
        const u = 1 - t;
        return (u * u * u * p0 + 3 * u * u * t * p1 + 3 * u * t * t * p2 + t * t * t * p3);
    }
    let x = t, a = 0, b = 1, guess = t;
    for (let i = 0; i < 5; i++) {
        const bezX = cubicBezier(guess, 0, c1x, c2x, 1);
        if (Math.abs(bezX - x) < 0.001)
            break;
        if (bezX < x)
            a = guess;
        else
...
function colorDistance(a, b) {
    ...
    const rgbA = hexToRgb(a);
    const rgbB = hexToRgb(b);
    return Math.sqrt(Math.pow(rgbA[0] - rgbB[0], 2) +
        Math.pow(rgbA[1] - rgbB[1], 2) +
        Math.pow(rgbA[2] - rgbB[2], 2));
}
function matchCircles(frameA, frameB) {
    const used = new Set();
    return frameB.map((b) => {
        let bestIdx = -1;
...
function interpolateRect(a, b, t, index) {
    var _a, _b, _c, _d;
    return {
        ...
        strokeWidth: lerp((_c = a.strokeWidth) !== null && _c !== void 0 ? _c : 0, (_d = b.strokeWidth) !== null && _d !== void 0 ? _d : 0, t),
    };
}
function interpolateCircle(a, b, t, index) {
    var _a, _b;
...
function calculatePathCenter(d) {
...
const Step3KeyframeAnimation = () => {
    ...
    const rects = Array.from({ length: maxRects }, (_, i) => interpolateRect(current.rects[i] || defaultRect, next.rects[i] || defaultRect, progress, i));
    const circles = Array.from({ length: maxCircles }, (_, i) => interpolateCircle(current.circles[i] || defaultCircle, next.circles[i] || defaultCircle, progress, i));
    const paths = Array.from({ length: maxPaths }, (_, i) => interpolatePath(current.paths[i] || defaultPath, next.paths[i] || defaultPath, progress));
    ...
    const pathFill = interpolateColor(pathA.fill || "#000000", pathB.fill || "#000000", progress);
    const pathStroke = pathA.stroke || pathB.stroke;
    const pathOpacity = lerp((_a = pathA.opacity) !== null && _a !== void 0 ? _a : 1, (_b = pathB.opacity) !== null && _b !== void 0 ? _b : 1, progress);
    const pathTransformA = PATH_TRANSFORMS[frame];
    const pathTransformB = PATH_TRANSFORMS[nextFrame];
    const interpTransform = interpolateTransform(pathTransformA, pathTransformB, progress);
    return (<div style={{ position: "relative", width: 260, height: 312 }}>
      <svg width={260} height={312} viewBox="0 0 260 312" fill="none">
        ...
        <g transform={`translate(${interpTransform.x},${interpTransform.y}) scale(${interpTransform.scale})`}>
          <path d={pathD} fill={pathFill} stroke={pathStroke} opacity={pathOpacity}/>
        </g>
      </svg>
    </div>);
};
exports.default = Step3KeyframeAnimation;
