...
exports.evaluateColorSystem = evaluateColorSystem;
...
function getEmpiricalBestContrast(n) {
    if (EMPIRICAL_BEST_CONTRAST[n])
        return EMPIRICAL_BEST_CONTRAST[n];
    ...
    for (let i = keys.length - 1; i >= 0; i--) {
        if (n > keys[i])
            return EMPIRICAL_BEST_CONTRAST[keys[i]];
    }
 ...
 */
function evaluateColorSystem(swatches) {
    const details = [];
    // 1. Swatch count
    ...
    if (swatchCount <= 9)
        swatchCountScore = 0;
    else if (swatchCount >= 10 && swatchCount <= 20)
        swatchCountScore = 1;
    else if (swatchCount >= 21)
        swatchCountScore = 0.3;
    // Bonus for 16+ swatches (diminishing returns), but only allow perfect for 18, 19, 20
    if (swatchCount >= 16) {
        if (swatchCount === 16)
            swatchCountScore += 0.05;
        else if (swatchCount === 17)
            swatchCountScore += 0.08;
        else if (swatchCount === 18)
            swatchCountScore += 0.1;
        else if (swatchCount === 19)
            swatchCountScore += 0.11;
        else if (swatchCount === 20)
            swatchCountScore += 0.12;
        else if (swatchCount >= 21)
            swatchCountScore = Math.min(swatchCountScore, 0.95); // cap for 21+
    }
    details.push(`Swatch count: ${swatchCount} (score: ${swatchCountScore.toFixed(2)})`);
    // 2. Evenness of steps (Gaussian ideal)
    const lValues = swatches.map((hex) => (0, colorUtils_1.hexToLabLightness)(hex));
    const steps = [];
    ...
    const meanIdealStep = idealSteps.reduce((a, b) => a + b, 0) / idealSteps.length;
    const expError = steps.reduce((acc, s, i) => acc + Math.exp(Math.abs(s - idealSteps[i]) / meanIdealStep), 0) / steps.length;
    let evennessScore = 1 / expError;
    evennessScore = Math.max(0, Math.min(1, evennessScore));
    details.push(`Evenness of steps (Gaussian ideal, exponential penalty): expError=${expError.toFixed(2)}, score=${evennessScore.toFixed(2)}`);
    details.push(`Actual steps: [${steps.map((s) => s.toFixed(2)).join(", ")}]`);
    details.push(`Ideal steps: [${idealSteps.map((s) => s.toFixed(2)).join(", ")}]`);
    // 3. Balance around 50
    ...
    details.push(`Balance around 50: light=${lightCount}, dark=${darkCount}, score=${balanceScore.toFixed(2)}`);
    ...
    for (let i = 0; i < lValues.length; i++) {
        ...
        if (currentL === 50)
            continue; // Skip the middle value
    ...
    const maxPossiblePairs = Math.floor((swatchCount - (lValues.includes(50) ? 1 : 0)) / 2);
    const symmetryScore = maxPossiblePairs > 0 ? symmetricPairs / maxPossiblePairs : 0;
    details.push(`Symmetry of steps: found ${symmetricPairs} symmetric pairs out of ${maxPossiblePairs} possible (score: ${symmetryScore.toFixed(2)})`);
    ...
    for (let i = 0; i < swatchCount; i++) {
        for (let j = 0; j < swatchCount; j++) {
            if (i === j)
                continue; // Omit same-color pairs
            const fg = swatches[i];
            const bg = swatches[j];
            const ratio = (0, colorUtils_1.calculateContrastRatio)(fg, bg);
            totalCombos++;
            ...
            if (passesAA)
                wcagAAPassing++;
            debugPairs.push(`${fg} on ${bg}: ratio=${ratio.toFixed(2)}, passesA=${passesA}, passesAA=${passesAA}`);
        }
    }
    const wcagAScore = totalCombos > 0 ? wcagAPassing / totalCombos : 0;
    const wcagAAScore = totalCombos > 0 ? wcagAAPassing / totalCombos : 0;
    details.push(`WCAG A combos: ${wcagAPassing}/${totalCombos} (${(wcagAScore * 100).toFixed(1)}%)`);
    details.push(`WCAG AA combos: ${wcagAAPassing}/${totalCombos} (${(wcagAAScore * 100).toFixed(1)}%)`);
    ...
    const normalizedContrastScore = empiricalBest > 0 ? Math.min(contrastScore / empiricalBest, 1) : 0;
    details.push(`Contrast score (60% AA, 40% A): ${contrastScore.toFixed(3)}`);
    details.push(`Normalized contrast score (empirical best = ${empiricalBest}): ${normalizedContrastScore.toFixed(3)}`);
    details.push(`Note: 1.0 means as good as mathematically possible for a full-range ramp of this size.`);
    // Bundled scores
    const visualQualityScore = (swatchCountScore * 0.25 +
        evennessScore * 0.45 +
        balanceScore * 0.1 +
        symmetryScore * 0.2) *
        100;
    details.push(`Visual Quality Score: ${visualQualityScore.toFixed(1)}`);
    details.push(`Accessibility Score (normalized contrast): ${(normalizedContrastScore * 100).toFixed(1)}`);
    // 7. Overall score (weighted average of all components), multiplied by 100
    const overallScore = (swatchCountScore * 0.15 +
        evennessScore * 0.27 +
        balanceScore * 0.06 +
        symmetryScore * 0.12 +
